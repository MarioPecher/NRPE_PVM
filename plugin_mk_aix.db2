...
function query_instance {
    ...
    DBS=\\\$(db2 list database directory | grep -p Indirect|grep 'Database name' | awk '{ print \\\$NF }')
    ...
    for DB in \\\$DBS; do
            ...
            echo "[[[$INSTANCE:\\\$DB]]]"
        db2 "SELECT tbsp_name, tbsp_type, tbsp_state, case when TBSP_CONTENT_TYPE = 'ANY' and TBSP_PAGE_SIZE=4096 then 67108864 when TBSP_CONTENT_TYPE = 'ANY' and TBSP_PAGE_SIZE=8192 then 134217728 when TBSP_CONTENT_TYPE = 'ANY' and TBSP_PAGE_SIZE=16384 then 268435456  when TBSP_CONTENT_TYPE = 'ANY' and TBSP_PAGE_SIZE=32768 then 536870912       when TBSP_CONTENT_TYPE <> 'ANY' and TBSP_PAGE_SIZE=4096 then 8589934592 when TBSP_CONTENT_TYPE <> 'ANY' and TBSP_PAGE_SIZE=8192 then 17179869184 when TBSP_CONTENT_TYPE <> 'ANY' and TBSP_PAGE_SIZE=16384 then 34359738368 when TBSP_CONTENT_TYPE <> 'ANY' and TBSP_PAGE_SIZE=32768 then 68719476736 End as tbsp_usable_size_kb, tbsp_total_size_kb, tbsp_used_size_kb, tbsp_free_size_kb FROM sysibmadm.tbsp_utilization WHERE tbsp_type = 'DMS' UNION ALL SELECT tu.tbsp_name, tu.tbsp_type, tu.tbsp_state, case when tu.TBSP_CONTENT_TYPE = 'ANY' and tu.TBSP_PAGE_SIZE=4096 then 67108864 when tu.TBSP_CONTENT_TYPE = 'ANY' and tu.TBSP_PAGE_SIZE=8192 then 134217728 when tu.TBSP_CONTENT_TYPE = 'ANY'and tu.TBSP_PAGE_SIZE=16384 then 268435456 when tu.TBSP_CONTENT_TYPE = 'ANY' and tu.TBSP_PAGE_SIZE=32768 then 536870912         when tu.TBSP_CONTENT_TYPE <> 'ANY' and tu.TBSP_PAGE_SIZE=4096 then 858993459 when tu.TBSP_CONTENT_TYPE <> 'ANY' and tu.TBSP_PAGE_SIZE=8192 then 17179869184  when tu.TBSP_CONTENT_TYPE <> 'ANY' and tu.TBSP_PAGE_SIZE=16384 then 34359738368      when tu.TBSP_CONTENT_TYPE <> 'ANY' and tu.TBSP_PAGE_SIZE=32768 then 68719476736 End as tbsp_usable_size_kb, tu.tbsp_total_size_kb, tu.tbsp_used_size_kb, (cu.fs_total_size_kb - cu.fs_used_size_kb) AS tbsp_free_size_kb FROM sysibmadm.tbsp_utilization tu INNER JOIN ( SELECT tbsp_id, 1 AS fs_total_size_kb, 0 AS fs_used_size_kb FROM sysibmadm.container_utilization WHERE (fs_total_size_kb IS NULL OR fs_used_size_kb IS NULL) GROUP BY tbsp_id) cu ON (tu.tbsp_type = 'SMS' AND tu.tbsp_id = cu.tbsp_id) UNION ALL SELECT tu.tbsp_name, tu.tbsp_type, tu.tbsp_state, case when tu.TBSP_CONTENT_TYPE = 'ANY' and tu.TBSP_PAGE_SIZE=4096 then 67108864 when tu.TBSP_CONTENT_TYPE = 'ANY' and tu.TBSP_PAGE_SIZE=8192 then 134217728 when tu.TBSP_CONTENT_TYPE = 'ANY' and tu.TBSP_PAGE_SIZE=16384 then 268435456 when tu.TBSP_CONTENT_TYPE = 'ANY' and tu.TBSP_PAGE_SIZE=32768 then 536870912 when tu.TBSP_CONTENT_TYPE <> 'ANY' and tu.TBSP_PAGE_SIZE=4096 then 8589934592 when tu.TBSP_CONTENT_TYPE <> 'ANY' and tu.TBSP_PAGE_SIZE=8192 then 17179869184       when tu.TBSP_CONTENT_TYPE <> 'ANY' and tu.TBSP_PAGE_SIZE=16384 then 34359738368 when tu.TBSP_CONTENT_TYPE <> 'ANY' and tu.TBSP_PAGE_SIZE=32768 then 68719476736      End as tbsp_usable_size_kb, tu.tbsp_total_size_kb, tu.tbsp_used_size_kb, (cu.fs_total_size_kb - cu.fs_used_size_kb) AS tbsp_free_size_kb FROM sysibmadm.tbsp_utilization tu INNER JOIN       ( SELECT tbsp_id, SUM(fs_total_size_kb) AS fs_total_size_kb, SUM(fs_used_size_kb) AS fs_used_size_kb FROM sysibmadm.container_utilization WHERE (fs_total_size_kb IS NOT NULL AND fs_used_size_kb IS NOT NULL) GROUP BY tbsp_id) cu ON (tu.tbsp_type = 'SMS' AND tu.tbsp_id = cu.tbsp_id)" | awk '{print \\\$1" "\\\$2" "\\\$3" "\\\$4" "\\\$5" "\\\$6" "\\\$7}' | sed -e '/^[ ]*$/d' -e '/^-/d' -e '/selected/d'    
	echo "<<<db2_counters>>>"
            echo "TIMESTAMP $NOW"
            ...
            db2 list applications for db \\\$DB | grep -v Auth | grep -v Name | sed -e '/^$/d' | wc -l | tr -d ' '
            ...
            if compare_version_greater_equal \\\$VERSION_NUMBER 10.5; then
                ...
                db2 -x "select SQLM_ELM_LAST_BACKUP from table(SNAPSHOT_DATABASE( cast( null as VARCHAR(255)), cast(null as int))) as ref where DB_NAME ='\\\$DB'" | grep -v "selected." | tail -n 1
            fi
